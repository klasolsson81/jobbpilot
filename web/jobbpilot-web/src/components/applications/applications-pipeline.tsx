...
interface ApplicationsPipelineProps {
  ...
  // ApplicationRow förblir server-renderbar (CTO punkt 4). Den passas in som
  // render-prop så raderna renderas server-side och bara fästs in i client-
  // öns DOM via RSC-payload — ApplicationsPipeline äger aldrig rad-utseendet.
  renderRow: (application: ApplicationDto) => ReactNode;
}
...
function PipelineSection({ group, renderRow }: PipelineSectionProps) {
  ...
  return (
    <section id={`status-${group.status}`} aria-label={label} className="scroll-mt-6">
      ...
      {open && (
        ...
        >
          {group.applications.map((app) => renderRow(app))}
        </div>
      )}
    </section>
  );
...
/**
 * Client-ö för /ansokningar-listan (CTO-beslut a8e269eb8dce9d541). Tar
 * serialiserad pipeline-data via props och äger tre interaktioner:
 *
 ...
 *
 * page.tsx förblir RSC och äger auth/error/total===0-tomtillstånd.
 * ApplicationRow förblir server-renderbar (passas in via renderRow).
 */
