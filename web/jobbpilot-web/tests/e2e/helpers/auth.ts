 ...
 *   §13.1 "Känsligt").
 * - Test-domänen är `e2e.jobbpilot.test` — RFC 6761 reserverar `.test` TLD
...
function assertSafeBaseURL(url: string): void {
  ...
  // av `https://localhost.evil.com/`, `https://prod.jobbpilot.se/?path=staging` osv.
  let host: string;
  try {
  ...
  const allowed =
    host === "localhost" ||
    host === "127.0.0.1" ||
    host === "staging.jobbpilot.se" ||
    host === "dev.jobbpilot.se" ||
    host.endsWith(".staging.jobbpilot.se") ||
    host.endsWith(".dev.jobbpilot.se");
  if (!allowed) {
    throw new Error(
      `E2E-helper avbruten: misstänkt produktions-host "${host}" (URL: ${url}). ` +
        `Tillåtna hostnamn: localhost, 127.0.0.1, *.staging.jobbpilot.se, *.dev.jobbpilot.se. ` +
        `Se TD-11.`
    );
  }
}
