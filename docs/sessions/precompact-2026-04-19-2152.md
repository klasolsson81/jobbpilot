...
created: 2026-04-19T21:52:43+02:00
...
```
 M src/JobbPilot.Application/Common/Behaviors/AuthorizationBehavior.cs
...
?? src/JobbPilot.Application/Auth/
?? src/JobbPilot.Application/Common/Abstractions/IAuthenticatedRequest.cs
?? src/JobbPilot.Application/Common/Abstractions/IUserAccountService.cs
?? src/JobbPilot.Application/Common/Exceptions/UnauthorizedException.cs
?? src/JobbPilot.Application/JobSeekers/
?? src/JobbPilot.Infrastructure/Auth/UserAccountService.cs
?? tests/JobbPilot.Application.UnitTests/Auth/
```
...
b15cc32 test(arch): Identity-isolation-test + ApiFactory Redis + dev appsettings JWT-sektion
ee649c4 feat(infra): AppIdentityDbContext + ApplicationUser + identity-schema
6b452fb chore(deps): Identity + JwtBearer + Redis packages (Testcontainers.PostgreSql uppgraderad till 4.11.0)
5b2df7b docs(decisions): ADR 0014 — refresh tokens i DB + Redis för access-token jti
2728bea docs(decisions): ADR 0013 — separat AppIdentityDbContext
e7507f0 docs(decisions): ADR 0012 — auth-stack ASP.NET Core Identity + JWT
f534bc1 docs(session): session 7 — Fas E komplett (Application + API + 35 tester)
