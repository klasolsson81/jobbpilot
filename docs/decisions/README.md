...
| 0061 | Sök-perf-strategi: GIN trigram-index på `LOWER(title)` + `LOWER(description)` via migration F6P4aJobAdTrigramIndexes (Approach A, CTO-dom 2026-05-20); pg_trgm trusted RDS-extension, partial-filter `WHERE status='Active' AND deleted_at IS NULL`, ingen Application-lager-ändring (Clean Arch bevarad). Implementations-dom för ADR 0042 Beslut D skala-trigger (51k > 15k tröskel + 40s p95 bevisat). ADR 0060 Beslut 4 N+1 YAGNI betingat bevarad — re-mätning post-deploy avgör ev. amend (ej preemptive). | Accepted | 2026-05-20 | [0061-job-ad-search-perf-strategy.md](./0061-job-ad-search-perf-strategy.md) |
