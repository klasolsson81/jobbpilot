...
**Status:** **TD-13 FAS 3.5 — C4.2/C4.3/C4.4/C5/C6 KOD KOMPLETT & PUSHAD (`c291ad6`, STOPP I passerad: svit grön Domain 358/App 492/Migrate 6/arch 70/Worker 68/Api 344, security-auditor + code-reviewer GO). STOPP V DEPLOY `v0.2.17-dev` FAILED — root cause: dev-ECS (`ASPNETCORE_ENVIRONMENT=Production`) saknar `FieldEncryption:CmkKeyId`; C1-hotfix-validatorn hård-failade boot (fail-closed KORREKT — vägrade köra utan CMK, bekräftar security-auditor fail-closed-PASS prod-likt). ECS circuit-breaker rullade tillbaka rent → dev kör `v0.2.16-dev`, friskt. BLOCKARE = KMS-infra (CMK + task-def-secret + ECS-roll-IAM) = Terraform/IaC §9.2 architect-obligatorisk + Klas-GO. dotnet-architect designar KMS-IaC (rapport→Klas-GO före Terraform). TD-13 INTE arkiverad (ej grön). ADR 0049 Mekanik-not 6-reconciliation utkastad (§9.4-undantag, väntar Klas-granskning). Session 2026-05-19.**
