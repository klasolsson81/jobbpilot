...
**Status:** **F6 P4 SÖK-INFRASTRUKTUR-FIX BACKEND LEVERERAD 2026-05-20 (pending tag-push `v0.2.51-dev` + Klas-GO för deploy + manuell SyncPlatsbankenSnapshotJob-backfill).** Två problem fixade i samma batch per §9.6: **P1 q-search-perf** (40s→<2s mål via Approach A GIN trigram-index, CTO-dom 2026-05-20) + **P2 filter-bugg-ROT-ORSAK** (JobTechHit-POCO saknade Occupation/OccupationGroup/OccupationField/WorkplaceAddress-properties → JsonSerializer.Serialize(hit) producerade payload utan klassifikations-keys → generated columns NULL för alla 51,749 rader → ssyk/region-filter alltid 0 träffar). Discovery via autentiserad probe mot dev.jobbpilot.se. CTO valde A över B/C/D (Clean Arch bevarad, YAGNI, KISS, DRY). Levererat: migration `F6P4aJobAdTrigramIndexes` (pg_trgm extension + 2 functional GIN-index på lower(title)/lower(description) med partial Active+soft-delete-filter, raw SQL, EJ CONCURRENTLY pga EF-transaktion-wrapping), JobTechHit POCO-utvidgning (4 nya properties + 4 nya internal sealed sub-types snake_case JsonPropertyName per JobTech v2-schema), 6 nya unit-tester inkl. end-to-end round-trip-grind genom JobTechPayloadSanitizer mot exakt rotorsaks-vektor, ADR 0061 (Accepted) sök-perf-strategi. **Reviews:** code-reviewer 0/0/0 + security-auditor 0/0/0/0/0 — båda APPROVED. **Tester:** Domain 399 + Application Unit 532 (526+6 nya) + Architecture 70 + Api.IntegrationTests/ListJobAdsFilter 13 + MultiFilter 6 — alla gröna. **Pending Klas:** tag-push `v0.2.51-dev` → dev-deploy (migration auto-appli) → manuell snapshot-trigger för backfill (51k rader får populerad raw_payload-klassifikation) → verifierings-spotcheck (EXPLAIN ANALYZE + q-search <2s + ssyk-filter totalCount>0). F6 P4b SavedJobAds avblockerad post-Klas-GO. Session-logg `docs/sessions/2026-05-20-2340-f6-p4-sok-infrastruktur-fix.md`.
